// USPS codes, names, and FIPS ids for the 50 states + DC. The schema
// validates map keys against `code`; the map component joins to us-atlas
// topology by `fips`.
...
export const US_STATE_CODES = US_STATES.map((s) => s.code) as [string, ...string[]];

export const usStateByCode: ReadonlyMap<string, UsState> = new Map(US_STATES.map((s) => [s.code, s]));
export const usStateByFips: ReadonlyMap<string, UsState> = new Map(US_STATES.map((s) => [s.fips, s]));
